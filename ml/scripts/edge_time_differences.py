...
parser.add_argument('-p', '--min_pmids', help="Number of pmids needed for an edge to be considered valid", type=int, default=2)
parser.add_argument('-a', '--alpha', help="Set the alpha value for the ElasticNet Regression", type=float, default=0.1)
parser.add_argument('-w', '--weight', help="Set the weight factor for DWPC calculations", type=float, default=0.6)
parser.add_argument('-e', '--edge_replace', help='The edge to repalce with values from another year', type=str, default='ASSOCIATED_WITH_DOawDO')
parser.add_argument('-y', '--replace_year', help='The network year to grab the edge from', type=int, default=1950)
...
gs_treat = False
w = args.weight
edge_replace = args.edge_replace
replace_year = args.replace_year

if network_year == replace_year:
    print('Network Year and Replace Year are the same.  This will result in no difference in the network')
    exit()

test_params = os.path.join(edge_replace, 'from_year_{}'.format(replace_year))

if alpha != 0.1:
    test_params = os.path.join(test_params, 'alpha_{}'.format(alpha))
...
load_dir = os.path.join(base_dir, str(network_year))
replace_dir = os.path.join(base_dir, str(replace_year))
out_dir = os.path.join('../data/edge_time_differences', str(network_year), test_params)
n_jobs = 32
get_all_probas = True
...
replace_edges = gt.remove_colons(pd.read_csv(os.path.join(replace_dir, 'edges.csv')))
...
if min_pmids > 1:
    ...
    replace_edges = replace_edges.query('n_pmids >= @min_pmids').reset_index(drop=True)
...
if not gs_treat:
    ...
    edges = pd.concat([edges, gs_edges])[column_order].reset_index(drop=True)

print('Replacing Edge: {} with edge from year {}'.format(edge_replace, replace_year))

node_ids = list(set(nodes['id']))

remove_idx = edges.query('type == @edge_replace').index
num_orig = len(remove_idx)
edges = edges.drop(remove_idx).reset_index(drop=True)
add_edges = replace_edges.query('type == @edge_replace')
add_edges = add_edges.query('start_id in @node_ids and end_id in @node_ids')
num_replaced = len(add_edges)
edges = pd.concat([edges, add_edges]).sort_values(['type', 'start_id', 'end_id']).reset_index(drop=True)

print('Removed {} edges and added {}: {:.2%} of original value'.format(num_orig, num_replaced, num_replaced/num_orig))
...
neg_index = cd_df.query('status == 0').sample(n=num_neg, random_state=int(network_year)).index
...
classifier = LogitNet(alpha=alpha, n_jobs=n_jobs, min_lambda_ratio=1e-8, n_lambda=150, standardize=True,
              random_state=int(network_year)+1, scoring=scoring)
