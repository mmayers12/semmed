...
parser.add_argument('-d', '--dropout', help='The edge to dropout and test for', type=str, default='ASSOCIATED_WITH_DOawDO')
parser.add_argument('-r', '--dropout_rate', help='The percentage of edges that should be dropped out', type=float, default=1)
parser.add_argument('-n', '--dropout_seed', help='Random seed for selecitng which edges to drop', type=int, default=0)
...
gs_treat = False
w = args.weight
to_dropout = args.dropout
dropout_rate = args.dropout_rate
dropout_seed = args.dropout_seed

test_params = os.path.join(to_dropout, 'rate_{}'.format(dropout_rate), 'seed_{}'.format(dropout_seed))
...
out_dir = os.path.join('../data/edge_dropout_testing', str(network_year), test_params)
n_jobs = 32
get_all_probas = True
...
print('Dropping out edge: {} at rate {:%} with seed {}'.format(to_dropout, dropout_rate, dropout_seed))

dropout_idx = dropout_idx = edges.query('type == @to_dropout').sample(frac=dropout_rate, replace=False, random_state=dropout_seed).index
edges = edges.drop(dropout_idx).reset_index(drop=True)
...
if to_dropout == "TREATS_CDtDO" and (1.0 - dropout_rate) < .0001:
    prior = cd_df[['cd_id', 'do_id']]
    blacklist = []
else:
    # Extract prior
